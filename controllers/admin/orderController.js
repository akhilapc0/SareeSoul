  ...
  try {
    ...
    if (['Cancelled', 'Returned', 'ReturnRequested'].includes(item.itemStatus)) {
      return res.status(400).json({ 
        message: `Cannot update item with status '${item.itemStatus}'` 
      });
    }
    ...
    if (!validTransitions[item.itemStatus]?.includes(status)) {
      return res.status(400).json({ 
        message: `Invalid status transition from ${item.itemStatus} to ${status}` 
      });
    }
...
const handleItemRequest = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      console.log('Return Approval - Refund Calculation:');
      ...
      if ((order.paymentMethod === 'Razorpay' || order.paymentMethod === 'Wallet') && 
        ...
        console.log(`Refunded ₹${refundAmount} to wallet for returned item`);
      }
