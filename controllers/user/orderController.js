  ...
  try {
    ...
    if (!canCancel) {
      return res.status(400).json({ 
        message: 'Order cannot be cancelled. Some items are already delivered, cancelled, or returned.' 
      });
    }
    ...
    if ((order.paymentMethod === 'Razorpay' || order.paymentMethod === 'Wallet') && 
      ...
      const refundAmount = calculateOrderRefund(
        order.items,
        order.discount || 0,
        order.subtotal
      );

      console.log('Order Cancellation Refund Calculation:');
      console.log('Original Subtotal (after offers):', order.subtotal);
      console.log('Coupon Discount:', order.discount || 0);
      ...
      console.log(`Refunded ₹${refundAmount} to wallet for order ${order.orderId}`);
    }
...
const cancelOrderItem = async (req, res) => {
  try {
    ...
    if (!['Pending', 'Shipped'].includes(item.itemStatus)) {
      return res.status(400).json({ 
        message: `Cannot cancel item with status: ${item.itemStatus}` 
      });
    }
    ...
    const breakdown = getRefundBreakdown(item, order.discount || 0, order.subtotal);
    
    console.log('Item Cancellation Refund Calculation:');
    console.log('Item ID:', item._id);
    console.log('Item Price (offer):', item.price);
    console.log('Quantity:', item.quantity);
    console.log('Item Total:', breakdown.itemTotal);
    console.log('Coupon Share:', breakdown.couponShare);
    console.log('Refund Amount:', refundAmount);
    ...
    const updatedTotals = recalculateOrderAfterCancellation(order, itemId);
    order.subtotal = updatedTotals.newSubtotal;
    order.discount = updatedTotals.newDiscount;
    order.total = updatedTotals.newTotal;
    ...
    if ((order.paymentMethod === 'Razorpay' || order.paymentMethod === 'Wallet') && 
      ...
      console.log(`Refunded ₹${refundAmount} to wallet for item ${itemId}`);
    }
...
const returnItem = async (req, res) => {
  try {
    ...
    const potentialRefund = calculateItemRefund(
      item,
      order.discount || 0,
      order.subtotal
    );
...
const returnOrder = async (req, res) => {
  try {
    ...
    const potentialRefund = calculateOrderRefund(
      deliveredItems,
      order.discount || 0,
      order.subtotal
    );
    ...
    return res.json({ 
      message: "Return request for order submitted successfully", 
      order,
      potentialRefund: potentialRefund
    });
...
const downloadInvoice = async (req, res) => {
  try {
    ...
    order.items.forEach((item, index) => {
      ...
      doc.text(`₹${item.price}`, 420, yPosition + 10);
      doc.text(`₹${item.quantity * item.price}`, 480, yPosition + 10);
    ...
    doc.text('Subtotal (After Offers):', summaryBoxX, yPosition);
    doc.text(`₹${parseFloat(order.subtotal).toFixed(2)}`, summaryBoxX + 120, yPosition, { align: 'right', width: 75 });
    
    yPosition += 20;
    
    if (order.discount && order.discount > 0) {
      doc.fillColor('#28a745')
         .text('Coupon Discount:', summaryBoxX, yPosition);
      doc.text(`-₹${parseFloat(order.discount).toFixed(2)}`, summaryBoxX + 120, yPosition, { align: 'right', width: 75 });
      doc.fillColor('#000000');
      yPosition += 20;
    }
    ...
    doc.text(`₹${parseFloat(order.total).toFixed(2)}`, summaryBoxX + 120, yPosition, { align: 'right', width: 75 });
